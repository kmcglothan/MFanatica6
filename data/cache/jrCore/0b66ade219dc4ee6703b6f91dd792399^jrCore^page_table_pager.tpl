                ...
                <td nowrap="nowrap" class="page_table_pager_center">
                    {if $total_pages > 0}
                        {if strlen($page_select) > 0}
                            {$page_jumper} / {$total_pages} &nbsp;&nbsp;&nbsp; {$page_select} {jrCore_lang module="jrCore" id=143 default="per page"}
                        {else}
                            {$page_jumper} / {$total_pages}
                        {/if}
                    {else}
                        {if strlen($page_select) > 0}
                            {$page_jumper} &nbsp; {$page_select} {jrCore_lang module="jrCore" id=143 default="per page"}
                        {else}
                            {$page_jumper}
                        {/if}
                    {/if}
