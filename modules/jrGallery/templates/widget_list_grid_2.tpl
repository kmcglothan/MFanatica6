        ...
        {foreach from=$_items item="item"}
            ...
            <div class="col6{if $item@last || ($item@iteration % 2) == 0} last{/if}">
                 <a href="{jrGallery_get_gallery_image_url item=$item}" title="@{$item.profile_url}: {$item.gallery_alt_text}" target="_blank">
                     {jrCore_module_function function="jrImage_display" module="jrGallery" type="gallery_image" item_id=$item._item_id size="xxxlarge" crop="square" class="img_scale" style="margin:0" alt=$item.gallery_image_name}
                 </a>
            </div>
