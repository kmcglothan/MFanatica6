...
{if isset($_items)}
        ...
        <div class="container">
            ...
            <div class="col3{if ($item@iteration % 4) === 0} last{/if} relative">
                <div id="p{$item._item_id}" class="p5 center">
                    ...
                    <br><a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$item.gallery_title_url}/all">{$item.gallery_title}</a><br><a href="{$jamroom_url}/{$item.profile_url}" style="margin-bottom:10px">@{$item.profile_url}</a>

                    {if (jrCore_module_is_active('jrPhotoAlbum')) }
                        <script>
                            $(function() {
                                var mid = $('#m{$item._item_id}');
                                $('#p{$item._item_id}').hover(function() { mid.show(); }, function() { mid.hide(); } );
                            });
                        </script>
    ...
    {else}
        ...
        {foreach $_items as $item}
            ...
            <a href="{jrGallery_get_gallery_image_url item=$item}">{jrCore_module_function function="jrImage_display" module="jrGallery" type="gallery_image" item_id=$item._item_id size="medium" crop="auto" class="iloutline" alt=$gtitle width=112}</a>
        {/foreach}
                            ...
                            <div class="jr_gallery_row">
                                ...
                                <div class="mt10" style="padding-top:0">
                                    {if $item.quota_jrGallery_gallery_order != 'off'}
                                        {jrCore_list module="jrGallery" profile_id=$item._profile_id search1="gallery_title_url = `$item.gallery_title_url`" template=$template order_by="gallery_order numerical_asc" exclude_jrUser_keys="true" exclude_jrProfile_quota_keys="true" limit=5}
                                    {else}
                                        {jrCore_list module="jrGallery" profile_id=$item._profile_id search1="gallery_title_url = `$item.gallery_title_url`" template=$template order_by="_created desc" exclude_jrUser_keys="true" exclude_jrProfile_quota_keys="true" limit=5}
                                    {/if}
