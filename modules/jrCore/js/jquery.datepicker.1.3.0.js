/* https://github.com/glad/glDatePicker/tree/version-1.3-archive */
(function(c)
{
    var r = {calId: 0, cssName: "default", startDate: -1, endDate: -1, selectedDate: -1, showPrevNext: !0, allowOld: !0, showAlways: !1, position: "absolute"},
        j = {
            init: function(a)
            {
            ...
            }, update: function()
            {
                ...
                d.setHours(0, 0, 0, 0);
                var k                  =
                        d.getTime(), f = new Date(0);
                -1 != b.endDate && (f = new Date(b.endDate), /^\d+$/.test(b.endDate) && (f = new Date(d), f.setDate(f.getDate() + b.endDate)));
                f.setHours(0, 0, 0, 0);
                var f = f.getTime(), h = new Date(0);
                -1 != b.selectedDate && (h = new Date(b.selectedDate), /^\d+$/.test(b.selectedDate) && (h = new Date(d), h.setDate(h.getDate() + b.selectedDate)));
                h.setHours(0, 0, 0, 0);
                var h = h.getTime(), i = a.data("theDate"), i = -1 == i || "undefined" == typeof i ? d : i,
                    m = new Date(i);
                m.setDate(1);
                ...
                f = _dp_jan + ',' + _dp_feb + ',' + _dp_mar + ',' + _dp_apr + ',' + _dp_may + ',' + _dp_jun + ',' + _dp_jul + ',' + _dp_aug + ',' + _dp_sep + ',' + _dp_oct + ',' + _dp_nov + ',' + _dp_dec;
                f = f.split(",")[i.getMonth()] + " " + i.getFullYear();
                k = ("<div class='**'><table><tr>" + ("<td class='**-prevnext prev'>" + (h ? "\u25c4" : "") + "</td>") + "<td class='**-monyear' colspan='5'>{MY}</td>" + ("<td class='**-prevnext next'>" + (k ? "\u25ba" : "") + "</td>") + "</tr><tr class='**-dow'><td>" + _dp_sun + "</td><td>" + _dp_mon + "</td><td>" + _dp_tue + "</td><td>" + _dp_wed + "</td><td>" + _dp_thu + "</td><td>" + _dp_fri + "</td><td>" + _dp_sat + "</td></tr>" +
                d + "</table></div>").replace(/\*{2}/gi, "gldp-" + b.cssName).replace(/\{MY\}/gi, f);
                0 == c("#" + e).length && a.after(c("<div id='" + e + "'></div>").css({position: b.position, "z-index": b.zIndex, left: a.position().left, top: a.position().top + a.outerHeight(!0)}));
                e = c("#" + e);
                ...
                }).click(function(b)
                {
                    ...
                    if (null != d.onChange && "undefined" != typeof d.onChange) d.onChange(a, e, _dp_lng);
                    d.selectedDate = e;
                    j.hide.apply(a)
                })
            }
