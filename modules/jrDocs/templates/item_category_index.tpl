...
<div class="block">
    ...
    <div class="title">
        ...
        <h1>{jrCore_lang module="jrDocs" id="53" default="Documentation"} &raquo; {$category}</h1>
        <div class="breadcrumbs">
            {if isset($search_string_value)}
                <a href="{$jamroom_url}/{$profile_url}">{$profile_name}</a> &raquo; <a href="{$jamroom_url}/{$profile_url}/{$murl}">{jrCore_lang module="jrDocs" id="53" default="Documentation"}</a> &raquo; <a href="{$jamroom_url}/{$profile_url}/{$murl}/{$category_url}">{$category}</a> &raquo; <a href="{$jamroom_url}/{$profile_url}/{$murl}{$breadcrumb_url}">{jrCore_lang module="jrDocs" id="63" default="Search Results"}</a>
            {else}
                <a href="{$jamroom_url}/{$profile_url}">{$profile_name}</a> &raquo; <a href="{$jamroom_url}/{$profile_url}/{$murl}">{jrCore_lang module="jrDocs" id="53" default="Documentation"}</a> &raquo; <a href="{$jamroom_url}/{$profile_url}/{$murl}/{$category_url}">{$category}</a>
            {/if}
        </div>
    </div>
        ...
        {if isset($_items)}
            ...
            {foreach $_items as $_doc}
                <li data-id="{$_doc._item_id}">
                        {if $_doc.doc_level == "1" || (isset($_doc.doc_snippet) && strlen($_doc.doc_snippet) > 0)}
                        <span class="doc_indent_1"><h2><a href="{$jamroom_url}/{$profile_url}/{$murl}/{$_doc.doc_category_url}/{$_doc._item_id}/{$_doc.doc_title_url}">{$_doc.doc_title}</a></h2></span>
                        {elseif $_doc.doc_level == "2"}
                        <span class="doc_indent_2"><h3><a href="{$jamroom_url}/{$profile_url}/{$murl}/{$_doc.doc_category_url}/{$_doc._item_id}/{$_doc.doc_title_url}">{$_doc.doc_title}</a></h3></span>
                        {else}
                        <span class="doc_indent_3"><h4>&bull; <a href="{$jamroom_url}/{$profile_url}/{$murl}/{$_doc.doc_category_url}/{$_doc._item_id}/{$_doc.doc_title_url}">{$_doc.doc_title}</a></h4></span>
                        {/if}
                        {if isset($_doc.doc_snippet) && strlen($_doc.doc_snippet) > 0}
                            <br><div class="p10">&quot;... {$_doc.doc_snippet} ...&quot;</div>
                        {/if}
                    </li>
                {/foreach}
                </ul>
