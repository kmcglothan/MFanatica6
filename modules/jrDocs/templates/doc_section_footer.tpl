...
{if !empty($item._prev) || !empty($item._next)}
    ...
    {if is_array($_cats)}
        {$keys = array_keys($_cats)}
        {foreach $_cats as $key => $_doc}
            {if $_doc.doc_category_url == $item.doc_category_url}
                {$cur = array_search($item.doc_category_url, $keys)}
                {$_prev = $_cats[$keys[($cur-1)]]}
                {$_next = $_cats[$keys[($cur+1)]]}
            {/if}
        {/foreach}
    {/if}
        ...
        <div class="doc_pager">
            <div style="display:table-row">
                <div class="doc_pager_cell" style="width:5%;">
                    ...
                    {elseif is_array($_prev)}
                        <input type="button" value="&lt;" class="form_button" onclick="window.location='{$jamroom_url}/{$item.profile_url}/{$murl}/{$_prev.doc_category_url}'">
                    {/if}
                </div>
                <div class="doc_pager_cell" style="width:37%;">
                    ...
                    {elseif is_array($_prev)}
                        <a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$_prev.doc_category_url}">{$_prev.doc_category}</a>
                    {/if}
                </div>
                ...
                <div class="doc_pager_cell right" style="width:37%">
                    ...
                    {elseif is_array($_next)}
                        <a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$_next.doc_category_url}">{$_next.doc_category}</a>
                    {/if}
                </div>
                <div class="doc_pager_cell right" style="width:5%">
                    ...
                    {elseif is_array($_next)}
                        <input type="button" value="&gt;" class="form_button" onclick="window.location='{$jamroom_url}/{$item.profile_url}/{$murl}/{$_next.doc_category_url}'">
                    {/if}
                </div>
            </div>
        </div>
