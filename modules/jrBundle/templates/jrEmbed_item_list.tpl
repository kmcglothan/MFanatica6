            ...
            {foreach $_items as $key => $item}
                <tr class="{cycle values="page_table_row,page_table_row_alt"}">
                    ...
                    <td class="page_table_cell center" style="width:16%">
                        <a onclick="jrEmbed_load_module('jrBundle', 1, 'profile_url:{$item.profile_url}');">@{$item.profile_name}</a>
                    </td>
                    <td class="page_table_cell center" style="width:16%">
                        {if is_array($item.bundle_items)}
                            {$item.bundle_items|count} {jrCore_lang module="jrBundle" id=29}
                        {/if}</td>
                    <td class="page_table_cell center" style="width:16%">
                    ...
                    <td class="page_table_cell" style="width:10%">
                        <input type="button" class="form_button embed_form_button" value="{jrCore_lang module="jrEmbed" id="1" default="Embed this Media"}" onclick="jrBundle_insert_bundle({$item._item_id})">
                    </td>
                </tr>
            {/foreach}
        </table>
    ...
    <div class="container">
        <table class="page_table">
            <tr class="page_table_row">
                <td class="page_table_cell center" colspan="2">{jrCore_lang module="jrBundle" id="45" default="no item bundles were found"}</td>
            </tr>
        </table>
    </div>
...
<script type="text/javascript">
    function jrBundle_insert_bundle(bundle_id)
    {
        parent.tinymce.activeEditor.insertContent('[jrEmbed module="jrBundle" id="' + bundle_id + '"]');
