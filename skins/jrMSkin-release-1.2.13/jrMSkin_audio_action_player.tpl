...
$(document).ready(function(){
    ...
    var autoPlayCookie = jrReadCookie('jrMSkin_audio_autoplay');
    var volumeCookie = jrReadCookie('jrMSkin_audio_volume');
    ...
    $('#jp_container_{$uniqid}').bind($.jPlayer.event.volumechange, function(event){
        ...
        jrSetCookie('jrMSkin_audio_volume', jp_volume, 31);
    });

    $('#jp_container_{$uniqid} #jp-auto-play').click(function() {
        jrSetCookie('jrMSkin_audio_autoplay', 0, 31);
        if ($(this).is(':checked')) {
            jrSetCookie('jrMSkin_audio_autoplay', 1, 31);
        }
    });
});
</script>

<div class="jrMSkin_player audio" onclick="event.cancelBubble = true; if(event.stopPropagation) event.stopPropagation();">
                            ...
                            <ul class="jp-toggles">
                                <li><a class="jp-settings" tabindex="{jrCore_next_tabindex}"></a>
                                    <div class="jp-control-settings" style="display: none;">
                                        <ul>
                                            <input type="checkbox" id="jp-show" /> <span>{jrCore_lang skin="jrMSkin" id="114" default="Show Playlist"}</span><br>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
