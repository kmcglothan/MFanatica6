...
<div class="page_nav clearfix">
    <div class="breadcrumbs">
        ...
        {jrMSkin_breadcrumbs module="jrVideo" profile_url=$item.profile_url profile_name=$item.profile_name page="detail" item=$item}
    </div>
...
<div class="col8">
    <div class="box">
        {jrMSkin_sort template="icons.tpl" nav_mode="jrVideo" profile_url=$profile_url}
        <div class="box_body">
            <div class="wrap detail_section">
                ...
                {elseif $item.video_file_extension == 'm4v'}
                    {jrCore_media_player module="jrVideo" field="video_file" item=$item autoplay=$_conf.jrMSkin_auto_play}
                {/if}
                <div class="detail_box">
                    <div class="basic-info">
                        <div class="trigger"><span>{jrCore_lang skin="jrMSkin" id="115" default="Basic Info"}</span></div>
                        <div class="item" style="display: none; padding: 0; margin: 5px auto 0;">
                            <div style="display: table; width: 100%;">
                                <div class="header">
                                    <div>{jrCore_lang skin="jrMSkin" id=21 default="Album"}</div>
                                    <div>{jrCore_lang skin="jrMSkin" id=41 default="Category"}</div>
                                    <div>{jrCore_lang skin="jrMSkin" id=40 default="Created"}</div>
                                    <div>{jrCore_lang skin="jrMSkin" id=38 default="Plays"}</div>
                                </div>
                                <div class="details">
                    ...
                    {if strlen($item.video_description) > 0}
                        <div class="description">
                            <div class="trigger"><span>{jrCore_lang skin="jrMSkin" id="47" default="Description"}</span></div>
                ...
                <div class="action_feedback">
                    {jrMSkin_feedback_buttons module="jrVideo" item=$item}
                    {if jrCore_module_is_active('jrRating')}
                        <div class="rating" id="jrVideo_{$item._item_id}_rating">{jrCore_module_function
                            function="jrRating_form"
                            type="star"
                            module="jrVideo"
                            index="1"
                            item_id=$item._item_id
                            current=$item.video_rating_1_average_count|default:0
                            votes=$item.video_rating_1_number|default:0}</div>
                    {/if}
