...
{if isset($_conf.jrNovaLight_require_images) && $_conf.jrNovaLight_require_images == 'on'}
        ...
        {if isset($_conf.jrNovaLight_artist_quota) && $_conf.jrNovaLight_artist_quota > 0}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrNovaLight_artist_quota`" template="artists_row.tpl" require_image="profile_image" pagebreak=$_conf.jrNovaLight_default_artist_pagebreak page=$_post.p}
        {else}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" template="artists_row.tpl" require_image="profile_image" pagebreak=$_conf.jrNovaLight_default_artist_pagebreak page=$_post.p}
        {/if}

{else}

        {if isset($_conf.jrNovaLight_artist_quota) && $_conf.jrNovaLight_artist_quota > 0}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrNovaLight_artist_quota`" template="artists_row.tpl" pagebreak=$_conf.jrNovaLight_default_artist_pagebreak page=$_post.p}
        {else}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" template="artists_row.tpl" pagebreak=$_conf.jrNovaLight_default_artist_pagebreak page=$_post.p}
        {/if}
