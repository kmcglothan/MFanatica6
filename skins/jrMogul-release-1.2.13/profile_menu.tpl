{if isset($_items)}
    <ul id="horizontal">
        ...
        {if isset($_post.option) && strlen($_post.option) > 0}
            <li><a href="{$jamroom_url}/{$profile_url}">{jrCore_lang skin="jrMogul" id=1 default="Home"}</a></li>
        {else}
            <li class="active"><a href="{$jamroom_url}/{$profile_url}">{jrCore_lang skin="jrMogul" id=1 default="Home"}</a></li>
        {/if}
