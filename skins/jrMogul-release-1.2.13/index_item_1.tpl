{if isset($_items)}
    {foreach from=$_items item="item"}
        {jrMogul_process_item item=$item module=$_conf.jrMogul_list_1_type assign="_item"}
        {if $item.list_rank == '2'}
...
{else}
    <div class="no-items">
        ...
        <button class="form_button" style="display: block; margin: 2em auto;" onclick="jrCore_window_location('{$jamroom_url}/{$core_url}/skin_admin/global/skin={$_conf.jrCore_active_skin}/section=list+1')">{jrCore_lang skin="jrMogul" id="148" default="Edit Configuration"}</button>
    </div>
{/if}