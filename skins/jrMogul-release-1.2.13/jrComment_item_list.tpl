                ...
                {jrCore_get_uploaded_files module="jrComment" item=$item field="comment_file"}
                    ...
                    <div>
                        {if jrUser_is_logged_in() && $_conf.jrComment_threading == 'on'}
                            ...
                            <a href="#" onclick="jrMogul_reply_to({$item._item_id}, '{$item.user_name|addslashes}','#{$item.comment_module}_{$item.comment_item_id}_comments' )">
                                {jrCore_lang skin="jrMogul" id=80 default="Reply"}</a>
                            &nbsp;&nbsp;
                        {/if}
