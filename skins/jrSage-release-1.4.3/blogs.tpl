            ...
            <div class="body_1 mr5">
                ...
                <div class="body_2">
                    {if isset($_conf.jrSage_profile_ids) && strlen($_conf.jrSage_profile_ids) > 0}
                        {jrCore_list module="jrProfile" order_by="_profile_id asc" profile_id=$_conf.jrSage_profile_ids template="index_artists_row.tpl" limit="3"}
                    {else}
                        {if isset($_conf.jrSage_require_images) && $_conf.jrSage_require_images == 'on'}
                            {jrCore_list module="jrProfile" order_by="_profile_id random" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrSage_artist_quota`" template="index_artists_row.tpl" limit="3" require_image="profile_image"}
                        {else}
                            {jrCore_list module="jrProfile" order_by="_profile_id random" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrSage_artist_quota`" template="index_artists_row.tpl" limit="3"}
                        {/if}
                    {/if}
                </div>
            </div>
    ...
    <div class="row">
        ...
        <div class="col9">
            <div class="body_1 mr5">
                ...
                <div class="body_3">
                    {jrCore_list module="jrBlog" template="index_blogs.tpl" pagebreak=$_conf.jrSage_default_blog_pagebreak page=$_post.p}
                </div>
