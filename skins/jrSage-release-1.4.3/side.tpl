...
{if strlen($tag_cloud) > 0}
    ...
    <div class="body_2 center">
        {$tag_cloud}
    </div>
{/if}

{* SITE RADIO *}
{if isset($_conf.jrSage_show_radio) && $_conf.jrSage_show_radio == 'on'}
    {if isset($_conf.jrSage_radio_title) && strlen($_conf.jrSage_radio_title) > 0}
        {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.jrSage_radio_title`" limit="1" tpl_dir="jrSage" template="index_radio.tpl"}
    {else}
        {if jrUser_is_master()}
            ...
            <div class="body_2 normal p20">
                Admin Note:&nbsp;<a href="{$jamroom_url}/core/skin_admin/global/skin=jrSage">Settings</a> <b>"Radio Title"</b> is not set!
            </div>
        {/if}
    {/if}
{/if}
...
{* SITE CHANNEL *}
{if isset($_conf.jrSage_show_tv) && $_conf.jrSage_show_tv == 'on'}
    {if isset($_conf.jrSage_tv_title) && strlen($_conf.jrSage_tv_title) > 0}
        {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.jrSage_tv_title`" tpl_dir="jrSage" template="index_channel.tpl"}
    {else}
        {if jrUser_is_master()}
...
{if jrCore_module_is_active('jrComment')}
    {jrCore_list module="jrComment" order_by="_created desc" template="side_comments.tpl" limit="10" assign="SIDE_COMMENTS"}
{/if}
...
{if $_conf.jrSage_ads_off != 'on'}
    ...
    <div class="body_3 center mb10">
        ...
        {elseif isset($_conf.jrSage_side_ad) && strlen($_conf.jrSage_side_ad) > 0}
            {$_conf.jrSage_side_ad}
        {else}
