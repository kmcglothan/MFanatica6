...
{* FEATURED LIST FUNCTION *}
    {if isset($_conf.jrProJam_featured_member) && $_conf.jrProJam_featured_member > 0}
        {jrCore_list module="jrProfile" limit="1" search1="_profile_id = `$_conf.jrProJam_featured_member`" template=$feat_mem_today}
    {else}
        {if isset($_conf.jrProJam_require_images) && $_conf.jrProJam_require_images == 'on'}
            {if isset($_conf.jrProJam_member_quota) && $_conf.jrProJam_member_quota > 0}
                {jrCore_list module="jrProfile" order_by="_created random" limit="1" quota_id=$_conf.jrProJam_member_quota search1="profile_active = 1" template=$feat_mem_today require_image="profile_image"}
            {else}
        ...
        {else}
            {if isset($_conf.jrProJam_member_quota) && $_conf.jrProJam_member_quota > 0}
                {jrCore_list module="jrProfile" order_by="_created random" limit="1" quota_id=$_conf.jrProJam_member_quota search1="profile_active = 1" template=$feat_mem_today}
            {else}
