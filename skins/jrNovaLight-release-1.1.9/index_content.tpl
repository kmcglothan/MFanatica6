{if isset($_items)}
    ...
    <div class="item">
        <a href="{$jamroom_url}/article/{$item._item_id}/{$item.page_title_url}">{jrCore_module_function function="jrImage_display" module="jrUser" type="user_image" class="iloutline" item_id=$item._user_id size="xsmall" alt=$item.user_name style="float:right;margin-left:8px;"}</a>
        <h3><a href="{$jamroom_url}/article/{$item._item_id}/{$item.page_title_url}">{$item.page_title}</a></h3><br>
