...
<div class="inner">
    ...
    {else}
        {if $_post.option == 'category' && isset($_post._1)}
            {jrCore_list module="jrBlog" order_by="_created asc" search1="blog_category_url = `$_post._1`" template="blogs_row.tpl" pagebreak=$_conf.jrNovaLight_default_pagebreak page=$_post.p pager=true}
        ...
        {else}
            {jrCore_list module="jrBlog" order_by=$order_by template="blogs_row.tpl" pagebreak=$_conf.jrNovaLight_default_pagebreak page=$_post.p pager=true}
        {/if}
    {/if}
</div>
