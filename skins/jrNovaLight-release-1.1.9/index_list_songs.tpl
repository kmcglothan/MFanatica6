{if isset($_items)}
  ...
  <div style="display:table">
      <div style="display:table-row">
          ...
          <div class="media_title p5" style="display:table-cell;vertical-align:middle">
              <a href="{$jamroom_url}/{$item.profile_url}/{$_params.module_url}/{$item._item_id}/{$item.audio_title_url}" title="{$item.audio_title}">{if strlen($item.audio_title) > 25}{$item.audio_title|truncate:25false}{else}{$item.audio_title}{/if}</a><br>
  ...
  {/foreach}
    ...
    <div class="center p10">
        <input type="button" value="{jrCore_lang module="jrNovaLight" id="41" default="more"} {jrCore_lang module="jrNovaLight" id="31" default="top"} {jrCore_lang module="jrNovaLight" id="13" default="songs"}" class="form_button" onclick="jrLoad('#top_songs','{$info.page_base_url}/p={$info.next_page}');$('html, body').animate({ scrollTop: $('#tsongs').offset().top -100 }, 'slow');return false;">
    </div>
        {elseif $info.page >= 2}
    <div class="block">
