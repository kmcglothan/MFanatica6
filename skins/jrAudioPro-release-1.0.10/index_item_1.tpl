        ...
        {if strlen($item.audio_title) > 0}
            ...
            <div class="index_item">
                <div class="wrap">
                    ...
                    <span>{jrCore_lang skin="jrAudioPro" id="34" default="by"} {$item.profile_name}</span>
...
{else}
    <div class="no-items">
        ...
        {if $_conf.jrAudioPro_require_price == 'on'}
            {jrCore_lang skin="jrAudioPro" id="63" default="This list currently requires items to have a price set."}
        {/if}
        <button class="form_button" style="display: block; margin: 2em auto;" onclick="jrCore_window_location('{$jamroom_url}/{$core_url}/skin_admin/global/skin={$_conf.jrCore_active_skin}/section=List+1')">{jrCore_lang skin="jrAudioPro" id="64" default="Edit Configuration"}</button>
    </div>
{/if}