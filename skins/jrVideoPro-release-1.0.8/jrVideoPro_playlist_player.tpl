<script type="text/javascript">
    $(document).ready(function() {
        ...
        var autoPlayCookie = jrReadCookie('jrVideoPro_video_autoplay');
        var volumeCookie = jrReadCookie('jrVideoPro_video_volume');
        var muteCookie = jrReadCookie('jrVideoPro_auto_mute');
        ...
        var playlist_{$uniqid} = new jPlayerPlaylist({
            ...
            {foreach $media as $v}
            {
                ...
                poster: "{$jamroom_url}/{$vurl}/image/video_image/{$v.item_id}/1280/crop=" + tw +':'+ th,
        ...
        $('#jp_container_{$uniqid}').bind($.jPlayer.event.volumechange, function(event){
            ...
            jrSetCookie('jrVideoPro_video_volume', jp_volume, 31);
        });

        $('#jp_container_{$uniqid} #jp-auto-play').click(function() {
            jrSetCookie('jrVideoPro_video_autoplay', 0, 31);

            if ($(this).is(':checked')) {
                jrSetCookie('jrVideoPro_video_autoplay', 1, 31);
            }
        });
        $('#jp_container_{$uniqid} #jp-auto-mute').click(function() {
            jrSetCookie('jrVideoPro_auto_mute', 0, 31);
            if ($(this).is(':checked')) {
                jrSetCookie('jrVideoPro_auto_mute', 1, 31);
            }
        });
    });
</script>

<div class="jrVideoPro_player" onclick="event.cancelBubble = true; if(event.stopPropagation) event.stopPropagation();">
                            ...
                            <ul class="jp-toggles">
                                <li><a class="jp-settings" tabindex="{jrCore_next_tabindex}"></a>
                                    <div class="jp-control-settings" style="display: none;">
                                        <ul>
                                            <input type="checkbox" id="jp-show" /> <span>{jrCore_lang skin="jrVideoPro" id=32 default="Show Playlist"}</span><br>
                                            <input type="checkbox" id="jp-auto-play" /> <span>{jrCore_lang skin="jrVideoPro" id=39 default="Auto Play"}</span><br>
                                            <input type="checkbox" id="jp-auto-mute" /> <span>{jrCore_lang skin="jrVideoPro" id=40 default="Auto Mute"}</span>
                                        </ul>
                                    </div>
                                </li>
