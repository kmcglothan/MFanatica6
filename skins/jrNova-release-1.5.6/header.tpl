...
<div id="wrapper">
    <div id="content">
                ...
                <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="{if !jrCore_is_mobile_device() && !jrCore_is_tablet_device()}col5{else}col12 last{/if}">
                            <div id="main_logo">
                                {if jrCore_is_mobile_device() || jrCore_is_tablet_device()}
                                    {jrCore_image id="mmt" skin="jrNova" image="menu.png" alt="menu" style="max-width:28px;max-height:28px;"}
        ...
        {if isset($selected) && $selected == 'home'}
            ...
            {assign var="spot_onoff" value=$_conf.jrNova_index_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_spotlight_ids}
        ...
        {elseif isset($selected) && $selected == 'charts'}
            ...
            {if isset($tab_selected) && $tab_selected == 'v_charts'}
                ...
                {assign var="spot_onoff" value=$_conf.jrNova_video_chart_spotlight}
                {assign var="spot_ids" value=$_conf.jrNova_video_chart_spotlight_ids}

            {else}

                {assign var="spot_module" value="jrAudio"}
                {assign var="spot_require_image" value="audio_image"}
                {assign var="spot_onoff" value=$_conf.jrNova_audio_chart_spotlight}
                {assign var="spot_ids" value=$_conf.jrNova_audio_chart_spotlight_ids}
        ...
        {elseif isset($selected) && $selected == 'artists'}
            ...
            {assign var="spot_module" value="jrProfile"}
            {assign var="spot_require_image" value="profile_image"}
            {assign var="spot_onoff" value=$_conf.jrNova_artists_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_artists_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'songs'}

            {assign var="spot_module" value="jrAudio"}
            {assign var="spot_require_image" value="audio_image"}
            {assign var="spot_onoff" value=$_conf.jrNova_songs_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_songs_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'soundcloud'}

            {assign var="spot_module" value="jrSoundCloud"}
            {assign var="spot_onoff" value=$_conf.jrNova_soundcloud_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_soundcloud_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'videos'}

            {assign var="spot_module" value="jrVideo"}
            {assign var="spot_require_image" value="video_image"}
            {assign var="spot_onoff" value=$_conf.jrNova_video_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_video_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'youtube'}

            {assign var="spot_module" value="jrYouTube"}
            {assign var="spot_onoff" value=$_conf.jrNova_youtube_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_youtube_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'vimeo'}

            {assign var="spot_module" value="jrVimeo"}
            {assign var="spot_onoff" value=$_conf.jrNova_vimeo_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_vimeo_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'events'}

            {assign var="spot_module" value="jrEvent"}
            {assign var="spot_require_image" value="event_image"}
            {assign var="spot_onoff" value=$_conf.jrNova_event_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_event_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'members'}

            {assign var="spot_module" value="jrProfile"}
            {assign var="spot_require_image" value="profile_image"}
            {assign var="spot_onoff" value=$_conf.jrNova_members_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_members_page_spotlight_ids}

        {elseif isset($selected) && $selected == 'blogs'}

            {assign var="spot_module" value="jrBlog"}
            {assign var="spot_onoff" value=$_conf.jrNova_blog_page_spotlight}
            {assign var="spot_ids" value=$_conf.jrNova_blog_page_spotlight_ids}
        ...
        {if isset($selected) && $selected == 'blog'}
            ...
            {assign var="spot_quota_id" value="`$_conf.jrNova_artist_quota`,`$_conf.jrNova_member_quota`"}

        {elseif isset($selected) &&  $selected == 'members'}

            {assign var="spot_quota_id" value=$_conf.jrNova_member_quota}

        {else}

            {assign var="spot_quota_id" value=$_conf.jrNova_artist_quota}
            ...
            <div class="outer mb8">
                <div class="inner" style="padding-left:0;">
                    {if isset($spot_ids) && strlen($spot_ids) > 0}
                        ...
                        {/if}
                    {else}
                        {if (isset($spot_require_image) && strlen($spot_require_image) > 0) && (isset($_conf.jrNova_require_images) && $_conf.jrNova_require_images == 'on')}
