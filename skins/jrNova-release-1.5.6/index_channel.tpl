...
{if isset($_items)}
    {foreach from=$_items item="item"}
        <div class="item">
            <div class="inner leader capital">
                ...
                {/if}
                <span class="title">{jrCore_lang  skin=$_conf.jrCore_active_skin id="35" default="community"} {jrCore_lang  skin=$_conf.jrCore_active_skin id="44" default="TV"}</span>
            </div>
                        ...
                        <div class="p10" style="display: table;margin-left: 10px;">
                            <div style="display: table-row;">
                                ...
                                <div class="left middle" style="display: table-cell;padding-left: 15px;">
                                    <a onclick="popwin('{$jamroom_url}/channel_player/playlist_id={$item._item_id}/title={$item.playlist_title}/autoplay=true','channel_player',820,725,true);">{jrCore_image image="button_player_play.png" width="32" height="32" alt="{$item.playlist_title}" title="{$item.playlist_title}" onmouseover="$(this).attr('src','`$jamroom_url`/skins/`$_conf.jrCore_active_skin`/img/button_player_play_hover.png');" onmouseout="$(this).attr('src','`$jamroom_url`/skins/`$_conf.jrCore_active_skin`/img/button_player_play.png');"}</a>
                                </div>
                            </div>
                        </div>
                    </div>
