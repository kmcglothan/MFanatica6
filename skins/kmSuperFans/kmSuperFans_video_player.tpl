<script type="text/javascript">
    $(document).ready(function() {
        ...
        var autoPlayCookie = jrReadCookie('kmSuperFans_video_autoplay');
        var volumeCookie = jrReadCookie('kmSuperFans_video_volume');
        var muteCookie = jrReadCookie('kmSuperFans_auto_mute');
        ...
        $('#jp_container_{$uniqid}').bind($.jPlayer.event.volumechange, function(event){
            ...
            jrSetCookie('kmSuperFans_video_volume', jp_volume, 31);
        });

        $('#jp_container_{$uniqid} #jp-auto-play').click(function() {
            jrSetCookie('kmSuperFans_video_autoplay', 0, 31);

            if ($(this).is(':checked')) {
                jrSetCookie('kmSuperFans_video_autoplay', 1, 31);
            }
        });
        $('#jp_container_{$uniqid} #jp-auto-mute').click(function() {
            jrSetCookie('kmSuperFans_auto_mute', 0, 31);
            if ($(this).is(':checked')) {
                jrSetCookie('kmSuperFans_auto_mute', 1, 31);
            }
        });
    });
</script>

<div class="kmSuperFans_player" onclick="event.cancelBubble = true; if(event.stopPropagation) event.stopPropagation();">
                            ...
                            <ul class="jp-toggles">
                                <li><a class="jp-settings" tabindex="{jrCore_next_tabindex}"></a>
                                    <div class="jp-control-settings" style="display: none;">
                                        <ul>
                                            <input type="checkbox" id="jp-show" /> <span>{jrCore_lang skin="kmSuperFans" id=32 default="Show Playlist"}</span><br>
                                            <input type="checkbox" id="jp-auto-play" /> <span>{jrCore_lang skin="kmSuperFans" id=39 default="Auto Play"}</span><br>
                                            <input type="checkbox" id="jp-auto-mute" /> <span>{jrCore_lang skin="kmSuperFans" id=40 default="Auto Mute"}</span>
                                        </ul>
                                    </div>
                                </li>
