{if isset($_items) && is_array($_items)}
    {foreach $_items as $item}
        ...
        {if jrCore_checktype($item['user_birthdate'], 'number_nz')}
            <span>{jrCore_icon icon="birthday" size="16" color="ff5500"} {jrCore_lang skin="kmSuperFans" id=59 default="Birthday"} {$item.user_birthdate|jrCore_date_birthday_format:"%B %d"}</span>
        {/if}
    {/foreach}
    {if $online == '1'}
