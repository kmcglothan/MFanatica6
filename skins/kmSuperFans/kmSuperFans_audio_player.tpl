    ...
    {if is_array($media)}
        {foreach $media as $a}
            {if $a._item.$ext == 'mp3'}
            {
                ...
                {/if}
                poster: "{$jamroom_url}/{$a.module_url}/image/audio_image/{$a.item_id}/large/crop=audio"
            },
            {/if}
        {/foreach}
    {/if}
...
<div class="kmSuperFans_audio" onclick="event.cancelBubble = true; if(event.stopPropagation) event.stopPropagation();">
