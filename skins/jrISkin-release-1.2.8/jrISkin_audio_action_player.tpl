...
$(document).ready(function(){
    ...
    var autoPlayCookie = jrReadCookie('jrISkin_audio_autoplay');
    var volumeCookie = jrReadCookie('jrISkin_audio_volume');
    ...
    $('#jp_container_{$uniqid}').bind($.jPlayer.event.volumechange, function(event){
        ...
        jrSetCookie('jrISkin_audio_volume', jp_volume, 31);
    });

    $('#jp_container_{$uniqid} #jp-auto-play').click(function() {
        jrSetCookie('jrISkin_audio_autoplay', 0, 31);
        if ($(this).is(':checked')) {
            jrSetCookie('jrISkin_audio_autoplay', 1, 31);
        }
    });
});
</script>

<div class="jrISkin_player audio" onclick="event.cancelBubble = true; if(event.stopPropagation) event.stopPropagation();">
                            ...
                            <ul class="jp-toggles">
                                <li><a class="jp-settings" tabindex="{jrCore_next_tabindex}"></a>
                                    <div class="jp-control-settings" style="display: none;">
                                        <ul>
                                            <input type="checkbox" id="jp-show" /> <span>{jrCore_lang skin="jrISkin" id="114" default="Show Playlist"}</span><br>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
