...
<div class="inner">
    {if isset($_conf.jrNova_require_images) && $_conf.jrNova_require_images == 'on'}
        ...
        {if isset($_conf.jrNova_member_quota) && $_conf.jrNova_member_quota > 0}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrNova_member_quota`" tpl_dir="jrNova" template="members_row.tpl" require_image="profile_image" pagebreak=$_conf.jrNova_default_artist_pagebreak page=$_post.p}
        {else}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" tpl_dir="jrNova" template="members_row.tpl" require_image="profile_image" pagebreak=$_conf.jrNova_default_artist_pagebreak page=$_post.p}
        {/if}

    {else}

        {if isset($_conf.jrNova_artist_quota) && $_conf.jrNova_artist_quota > 0}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrNova_member_quota`" template="members_row.tpl" pagebreak=$_conf.jrNova_default_artist_pagebreak page=$_post.p}
        {else}
            {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" template="members_row.tpl" pagebreak=$_conf.jrNova_default_artist_pagebreak page=$_post.p}
        {/if}
