            ...
            <div class="row">
                ...
                <div class="col5">
                    <div class="center">
                        ...
                        <h3><span style="color: #FFF;">{jrCore_lang skin=$_conf.jrCore_active_skin id="112" default="By"}:</span> <a href="{$jamroom_url}/{$item.profile_url}">{$item.profile_name}</a></h3>
                        <br>
                    </div>
                </div>
                <div class="col7 last">
                    <div class="left p20">
                        <br>
                        <h3><span style="color: #FFF;">{jrCore_lang module="jrAudio" id="12" default="genre"}:</span> <span class="highlight-txt bold">{$item.audio_genre}</span></h3>
                        <br>
                        <br>
                        {if isset($item.audio_album) && strlen($item.audio_album) > 0}
                            <h3><span style="color: #FFF;">{jrCore_lang skin=$_conf.jrCore_active_skin id="64" default="album"}:</span> <a href="{$jamroom_url}/{$item.profile_url}/{$murl}/albums/{$item.audio_album_url}">{$item.audio_album}</a></h3>
                            <br>
                            <br>
                        {/if}
...
{if isset($_conf.jrProJamLight_favorite_song) && strlen($_conf.jrProJamLight_favorite_song) > 0}
    {jrCore_list module="jrAudio" tpl_dir="jrProJamLight" limit="1" search1="_item_id in `$_conf.jrProJamLight_favorite_song`" template=$fav_song_row}
{else}
    {if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
        {jrCore_list module="jrAudio" order_by="_profile_id random" tpl_dir="jrProJamLight" limit="1" template=$fav_song_row require_image="audio_image"}
    {else}
        {jrCore_list module="jrAudio" order_by="_profile_id random" limit="1" template=$fav_song_row}
    {/if}
{/if}
