                        ...
                        <div class="col12 last">
                            ...
                            <div class="body_1">
                                ...
                                {if isset($_conf.jrMediaPro_require_images) && $_conf.jrMediaPro_require_images == 'on'}
                                    {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
                                        {jrCore_list module="jrAudio" chart_field="audio_file_stream_count" chart_days="7" quota_id=$_conf.jrMediaPro_artist_quota search1="audio_genre like `$_post.search_string`" template="music_chart_row.tpl" require_image="audio_image" pagebreak=$_conf.jrMediaPro_default_pagebreak page=$_post.p}
                                    {else}
                                        {jrCore_list module="jrAudio" chart_field="audio_file_stream_count" chart_days="7" quota_id=$_conf.jrMediaPro_artist_quota template="music_chart_row.tpl" require_image="audio_image" pagebreak=$_conf.jrMediaPro_default_pagebreak page=$_post.p}
                                    {/if}
                                {else}
                                    {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
                                        {jrCore_list module="jrAudio" chart_field="audio_file_stream_count" chart_days="7" quota_id=$_conf.jrMediaPro_artist_quota search1="audio_genre like `$_post.search_string`" template="music_chart_row.tpl" pagebreak=$_conf.jrMediaPro_default_pagebreak page=$_post.p}
                                    {else}
                                        {jrCore_list module="jrAudio" chart_field="audio_file_stream_count" chart_days="7" quota_id=$_conf.jrMediaPro_artist_quota template="music_chart_row.tpl" pagebreak=$_conf.jrMediaPro_default_pagebreak page=$_post.p}
                                    {/if}
                                {/if}
                            </div>
                        </div>
