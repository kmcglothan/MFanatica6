...
<div class="body_1">
    ...
    {if $_post.option == 'exclusive' || $_post.option == 'Exclusive' || $_post.option == 'featured' || $_post.option == 'Featured'}
        &raquo;&nbsp;<a href="#newslist" onclick="jrLoad('#news_listing','{$jamroom_url}/news_list');$('html, body').animate({ scrollTop: $('#newslist').offset().top });return false;">{jrCore_lang skin=$_conf.jrCore_active_skin id="141" default="Reset"}</a>
    {/if}
</div>
...
{if isset($_conf.jrMediaProLight_blog_profile) && $_conf.jrMediaProLight_blog_profile > 0}
    {jrCore_list module="jrBlog" order_by="_created desc" search1="blog_category = `$news_category`" search2="_profile_id in `$_conf.jrMediaProLight_blog_profile`" template=$latest_news_template pagebreak="6" page=$_post.p}
{else}
    {jrCore_list module="jrBlog" order_by="_created desc" search1="blog_category = `$news_category`" search2="_user_id = 1" template=$latest_news_template pagebreak="6" page=$_post.p}
{/if}
