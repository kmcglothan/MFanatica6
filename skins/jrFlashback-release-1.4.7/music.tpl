                        ...
                        <div class="col12 last">
                            ...
                            <div class="alpha_title">&raquo;&nbsp;
                                ...
                                {foreach from=$alpha item="char"}
                                    <a href="{$jamroom_url}/music{if isset($_post.option) && strlen($_post.option) > 0}/{$_post.option}{else}/by_newest{/if}/{$char}">{$char}</a>&nbsp;
                                {/foreach}
                                &laquo;&nbsp;
                                <a href="{$jamroom_url}/music{if isset($_post.option) && strlen($_post.option) > 0}/{$_post.option}{/if}">{jrCore_lang skin=$_conf.jrCore_active_skin id="105" default="Reset"}</a>
                            </div>
                            <div class="body_3">
                                {if isset($_conf.jrFlashback_require_images) && $_conf.jrFlashback_require_images == 'on'}
                                    {jrCore_list module="jrAudio" order_by=$order_by tpl_dir="jrFlashback" template="music_row.tpl" require_image="audio_image" search="audio_title like `$_post._1`%" pagebreak=$_conf.jrFlashback_default_pagebreak page=$_post.p}
                                {else}
                                    {jrCore_list module="jrAudio" order_by=$order_by tpl_dir="jrFlashback" template="music_row.tpl" search="audio_title like `$_post._1`%" pagebreak=$_conf.jrFlashback_default_pagebreak page=$_post.p}
                                {/if}
                            </div>
                        </div>
                    </div>
