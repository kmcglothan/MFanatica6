...
{capture name="row_template" assign="new_members_template"}
    ...
    <div style="float:left; padding-top:9px;padding-bottom:9px;">
        {if $info.prev_page > 0}
        <span class="button-arrow-previous" onclick="jrLoad('#newest_members','{$info.page_base_url}/p={$info.prev_page}');$('html, body').animate({ scrollTop: $('#nmembers').offset().top -100 }, 'slow');">&nbsp;</span>
        {else}
        <span class="button-arrow-previous-off">&nbsp;</span>
        {/if}
        {if $info.next_page > 1}
        <span class="button-arrow-next" onclick="jrLoad('#newest_members','{$info.page_base_url}/p={$info.next_page}');$('html, body').animate({ scrollTop: $('#nmembers').offset().top -100 }, 'slow');">&nbsp;</span>
        {else}
...
{if isset($_conf.jrProJam_require_images) && $_conf.jrProJam_require_images == 'on'}
    {if isset($_conf.jrProJam_member_quota) && $_conf.jrProJam_member_quota > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJam_member_quota`" template=$new_members_template require_image="profile_image" pagebreak="3" page=$_post.p}
    {else}
...
{else}
    {if isset($_conf.jrProJam_member_quota) && $_conf.jrProJam_member_quota > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJam_member_quota`" template=$new_members_template pagebreak="3" page=$_post.p}
    {else}
