                ...
                <div class="row">
                    ...
                    <div class="col10 last">
                        <div class="left" style="padding-left:15px;">
                            ...
                            {/if} *}
                            <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="121" default="Name"}</span>: <a href="{$jamroom_url}/{$item.profile_url}" title="{$item.profile_name}"><span class="capital bold">{$item.profile_name}</span></a><br>
                            {if isset($item.profile_influences) && strlen($item.profile_influences) > 0}
                                <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="143" default="Influences"}</span>: <span class="highlight-txt bold">{$item.profile_influences}</span><br>
                            {/if}
                            {if isset($_post.option) && $_post.option == 'by_newest'}
                                <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="122" default="Joined"}</span>: <span class="highlight-txt bold">{$item._created|date_format:"%A, %B %e, %Y %I:%M:%S %p"}</span><br>
                            {/if}
                            <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="13" default="Songs"}</span>: <span class="highlight-txt bold">{$item.profile_jrAudio_item_count}</span><br>
                            <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="50" default="Views"}</span>: <span class="highlight-txt bold">{$item.profile_view_count}</span><br>
                            <br>
                            {if strlen($item.profile_bio) > 0}
                                <span class="capital">{jrCore_lang skin=$_conf.jrCore_active_skin id="118" default="About"}</span>:<br>
