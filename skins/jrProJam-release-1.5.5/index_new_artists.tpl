...
{if isset($_conf.jrProJam_require_images) && $_conf.jrProJam_require_images == 'on'}
    {if isset($_conf.jrProJam_artist_quota) && strlen($_conf.jrProJam_artist_quota) > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJam_artist_quota`" template=$new_artists_template require_image="profile_image" pagebreak="3" page=$_post.p}
    {else}
...
{else}
    {if isset($_conf.jrProJam_artist_quota) && strlen($_conf.jrProJam_artist_quota) > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJam_artist_quota`" template=$new_artists_template pagebreak="3" page=$_post.p}
    {else}
