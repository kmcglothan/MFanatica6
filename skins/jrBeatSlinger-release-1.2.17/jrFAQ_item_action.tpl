...
<div class="action_info">
    ...
    <div class="action_data">
        ...
        {if $item_mode == 'create'}
            <span class="action_desc"><a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$item._item_id}/{$item.audio_title_url}">
                    {jrCore_lang module="jrFAQ" id="11" default="Posted a new faq"}.</a></span><br>
        {else}

            <a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$item._item_id}/{$item.audio_title_url}" title="{$item.audio_title|jrCore_entity_string}">
