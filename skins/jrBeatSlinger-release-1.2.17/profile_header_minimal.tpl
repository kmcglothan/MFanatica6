{if isset($_post._1) && $_post._1 == 'profiles_followed' && jrCore_checktype($_post._profile_id, 'number_nz')}
    {$_prof = jrCore_db_get_item('jrProfile', $_post._profile_id, true)}
    {$profile_quota_id = $_prof.profile_quota_id}
    {$profile_header_image_size = $_prof.profile_header_image_size}
    {$profile_url = $_prof.profile_url}
    {$profile_name = $_prof.profile_name}
    {$_profile_id = $_prof._profile_id}
{elseif jrCore_checktype($item._profile_id, 'number_nz')}
...
{if $_conf.jrBeatSlinger_show_header == 'minimal'}
                ...
                <div class="media">
                    ...
                    {if jrCore_module_is_active("jrFollower")}
                        <div class="profile_data">
                            {jrBeatSlinger_stats assign="action_stats" profile_id=$_profile_id}
                            {jrCore_module_url module="jrFollower" assign="furl"}
                            {jrCore_module_url module="jrAction" assign="murl"}
                            <ul class="clearfix">
                                <li onclick="jrCore_window_location('{$jamroom_url}/{$profile_url}/{$furl}')"><span>{jrCore_lang skin="jrBeatSlinger" id="126" default="Followers"}</span>
                                    {$action_stats.followers}</li>
                                <li onclick="jrCore_window_location('{$jamroom_url}/{$profile_url}/{$furl}/profiles_followed')"><span>{jrCore_lang skin="jrBeatSlinger" id="145" default="Following"}</span>
                                    {$action_stats.following}</li>
                                <li onclick="jrCore_window_location('{$jamroom_url}/{$profile_url}/{$murl}/timeline')"><span>{jrCore_lang skin="jrBeatSlinger" id="144" default="Updates"}</span>
