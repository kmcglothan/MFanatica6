...
<div class="page_nav clearfix">
    <div class="breadcrumbs">
        ...
        {jrCelebrity_breadcrumbs module="jrGroup" profile_url=$item.profile_url profile_name=$item.profile_name page="detail" item=$item}
    </div>
...
<div class="row">
    <div class="col9">
        <div class="box">
            {jrCelebrity_sort template="icons.tpl" nav_mode="jrGroup" profile_url=$profile_url}
            <div class="box_body">
        ...
        <div class="box">
            <ul class="head_tab">
                <li>
                    <a href="#" title="{jrCore_lang module="jrGroup" id=1 default="Group"}">{jrCore_icon icon="forum" size="20" color="CCCCCC"}</a>
                </li>
                <div class="action_buttons dark">
                    {if jrGroup_member_has_access($item)}
                        {if $item.discuss_user_is_following == '1'}
                            {jrCore_module_function function="jrGroupDiscuss_follow_group_button" icon="site-hilighted" color="CCCCCC" group_id=$item._item_id size=20}
                        {else}
                            {jrCore_module_function function="jrGroupDiscuss_follow_group_button" icon="site" color="CCCCCC" group_id=$item._item_id size=20}
                        {/if}
                    {/if}
                    ...
                    {if jrCore_module_is_active('jrFeed')}
                        ...
                        <a href="{$jamroom_url}/{$furl}/{$ndurl}/limit=100/group_id={$item._item_id}">{jrCore_icon icon="rss" size=20 color="CCCCCC"}</a>
                    {/if}
                    {if jrProfile_is_profile_owner($item._profile_id) || jrGroup_get_user_config('jrGroupDiscuss', 'allowed', $item, $_user._user_id) == 'on'}
                        ...
                        <a href="{$jamroom_url}/{$ndurl}/create/group_id={$item._item_id}"
                           title="{$ttl|jrCore_entity_string}">{jrCore_icon icon="plus" size=20 color="CCCCCC"}</a>
                    {/if}
                </div>
            </ul>
            <div class="box_body">
                    ...
                    <div class="action_feedback" style="padding: 0">
                        {if jrGroup_member_has_access($item)}
                            {if !jrCore_module_is_active('jrComment') || !isset($item.group_wall) || $item.group_wall == 'off'}
                                {jrCelebrity_feedback_buttons module="jrGroup" comment=false item=$item}
                                {if jrCore_module_is_active('jrRating')}
                                    <div class="rating" id="jrGroup_{$item._item_id}_rating">{jrCore_module_function
                                        function="jrRating_form"
                                        type="star"
                                        module="jrGroup"
                                        index="1"
                                        item_id=$item._item_id
                                        current=$item.group_rating_1_average_count|default:0
                                        votes=$item.group_rating_1_number|default:0}</div>
                                {/if}
                                {jrCore_item_detail_features module="jrGroup" item=$item exclude="jrComment~item_comments,jrDisqus~disqus_comment"}
                            {else}
                                {jrCelebrity_feedback_buttons module="jrGroup" item=$item}
                                {if jrCore_module_is_active('jrRating')}
                                    <div class="rating" id="jrGroup_{$item._item_id}_rating">{jrCore_module_function
                                        function="jrRating_form"
                                        type="star"
    ...
    <div class="col3 last">
        <div class="box" style="padding-left:1em">
            <ul class="head_tab">
                <li>
                    <a href="#" title="{jrCore_lang module="jrGroup" id=149 default="Pages"}">{jrCore_icon icon="profile" size="20" color="CCCCCC"}</a>
                </li>
            </ul>
            <div class="box_body">
                <div class="wrap">
        ...
        {if jrCore_module_is_active('jrGroupPage')}
            <div class="box" style="padding-left:1em">
                <ul class="head_tab">
                    <li id="page_tab">
                        <a href="#" title="{jrCore_lang module="jrGroup" id=13 default="Members"}">{jrCore_icon icon="page" size="20" color="CCCCCC"}</a>
                    </li>
                    {if jrProfile_is_profile_owner($item._profile_id) || jrGroup_get_user_config('jrGroupPage', 'allowed', $item, $_user._user_id) == 'on'}
                        <div class="action_buttons">
                            ...
                            <a href="{$jamroom_url}/{$npurl}/create/group_id={$item._item_id}"
                               title="{$pttl|jrCore_entity_string}">{jrCore_icon icon="plus" size=20  color="CCCCCC"}</a>
                        </div>
                    {/if}
                </ul>
                <div class="box_body">
