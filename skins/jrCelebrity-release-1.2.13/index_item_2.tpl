{if isset($_items)}
    {foreach from=$_items item="item"}
        {jrCelebrity_process_item item=$item module=$_conf.jrCelebrity_list_2_type assign="_item"}
        {if $item.list_rank == '1'}
            <div class="col6">
        {/if}
...
{else}
    <div class="no-items">
        ...
        <button class="form_button" style="display: block; margin: 2em auto;" onclick="jrCore_window_location('{$jamroom_url}/{$core_url}/skin_admin/global/skin={$_conf.jrCore_active_skin}/section=list+2')">{jrCore_lang skin="jrCelebrity" id="148" default="Edit Configuration"}</button>
    </div>
{/if}