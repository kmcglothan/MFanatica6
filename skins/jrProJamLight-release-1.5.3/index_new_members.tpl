...
{if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
    {if isset($_conf.jrProJamLight_member_quota) && $_conf.jrProJamLight_member_quota > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_member_quota`" template=$new_members_template require_image="profile_image" pagebreak="3" page=$_post.p}
    {else}
...
{else}
    {if isset($_conf.jrProJamLight_member_quota) && $_conf.jrProJamLight_member_quota > 0}
        {jrCore_list module="jrProfile" order_by="_created desc" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_member_quota`" template=$new_members_template pagebreak="3" page=$_post.p}
    {else}
