                    ...
                    <div class="row">
                        <div class="col12 last">
                            ...
                            {if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
                                {if isset($_post.option) && $_post.option == 'most_viewed'}
                                    {jrCore_list module="jrProfile" chart_field="profile_view_count" chart_days="365" search1="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" search2="profile_name like `$_post._1`%" template="artists_row.tpl" require_image="profile_image" pagebreak=$_conf.jrProJamLight_default_artist_pagebreak page=$_post.p}
                                {else}
                                    {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" search3="profile_name like `$_post._1`%" template="artists_row.tpl" require_image="profile_image" pagebreak=$_conf.jrProJamLight_default_artist_pagebreak page=$_post.p}
                                {/if}
                            {else}
                                {if isset($_post.option) && $_post.option == 'most_viewed'}
                                    {jrCore_list module="jrProfile" chart_field="profile_view_count" chart_days="365" search1="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" search2="profile_name like `$_post._1`%" template="artists_row.tpl" pagebreak=$_conf.jrProJamLight_default_artist_pagebreak page=$_post.p}
                                {else}
                                    {jrCore_list module="jrProfile" order_by=$order_by search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" search3="profile_name like `$_post._1`%" template="artists_row.tpl" pagebreak=$_conf.jrProJamLight_default_artist_pagebreak page=$_post.p}
                                {/if}
                            {/if}
                            </div>
                        </div>
