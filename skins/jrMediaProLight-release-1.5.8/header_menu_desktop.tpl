        ...
        <ul id="menu">
            ...
            {else}
                ...
                {if $_conf.jrCore_maintenance_mode != 'on' || jrUser_is_master() || jrUser_is_admin()}
                    ...
                    {if isset($_conf.jrMediaProLight_forum_profile_url) && strlen($_conf.jrMediaProLight_forum_profile_url) > 0}
                        {assign var="forum_url" value=$_conf.jrMediaProLight_forum_profile_url}
                    {/if}
                    ...
                    {if isset($_conf.jrMediaProLight_docs_profile_url) && strlen($_conf.jrMediaProLight_docs_profile_url) > 0}
                        {assign var="doc_url" value=$_conf.jrMediaProLight_docs_profile_url}
                    {/if}
            ...
            {elseif jrCore_module_is_active('jrFoxyCart') && strlen($_conf.jrFoxyCart_api_key) > 0}
                {jrCore_lang skin=$_conf.jrCore_active_skin id="204" default="Cart" assign="ct"}
                {if jrUser_is_logged_in()}
                    <li>
                        <a href="{$_conf.jrFoxyCart_store_domain}/cart?cart=view">{jrCore_image image="cart.png" width="24" height="24" alt="{$ct}"}<span id="fc_minicart"><span id="fc_quantity" class="hl-4"></span></span></a>
                    </li>
                {/if}
            {/if}
