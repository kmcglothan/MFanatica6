        ...
        <div class="container">
            <div class="row">
                <div class="col8">
                    ...
                    {if jrUser_is_logged_in()}
                        ...
                        {if jrCore_module_is_active('jrPrivateNote')}
                            {if isset($_user.user_jrPrivateNote_unread_count) && $_user.user_jrPrivateNote_unread_count > 0}
                                <a href="{$jamroom_url}/note/notes" target="_top"><span class="page-welcome" style="padding:2px;">{jrCore_lang skin=$_conf.jrCore_active_skin id="103" default="In Box"}</span></a> ({$_user.user_jrPrivateNote_unread_count}) |
                            {else}
                    ...
                    {else}
                        <b>{jrCore_lang skin=$_conf.jrCore_active_skin id="7" default="Welcome Guest"}!</b> | <a href="{$jamroom_url}/user/login" target="_top"><span class="page-welcome" style="padding:2px;">{jrCore_lang skin=$_conf.jrCore_active_skin id="6" default="Login"}</span></a>
                    {/if}
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col6">
                    ...
                    <div id="main_logo">
                        {if jrCore_is_mobile_device() || jrCore_is_tablet_device()}
                            {jrCore_image id="mmt" skin="jrProJamLight" image="menu.png" alt="menu" style="max-width:28px;max-height:28px;"}
                ...
                <div class="col6 last">
                    ...
                    <div class="logo-ads">
                        {if $_conf.jrProJamLight_ads_off != 'on'}
                            ...
                            {elseif isset($_conf.jrProJamLight_top_ad) && strlen($_conf.jrProJamLight_top_ad) > 0}
                                {$_conf.jrProJamLight_top_ad}
                            {else}
        ...
        <div class="logo-ads" style="float:none;padding:10px;text-align:center;">
            {if $_conf.jrProJamLight_ads_off != 'on'}
                ...
                {elseif isset($_conf.jrProJamLight_top_ad) && strlen($_conf.jrProJamLight_top_ad) > 0}
                    {$_conf.jrProJamLight_top_ad}
                {else}
                    <a href="https://www.jamroom.net/" target="_blank">{jrCore_image image="468x60_banner.png" alt="468x60 Ad" title="Get Jamroom5!" class="img_scale" style="max-width:468px;max-height:60px;margin:0 auto;"}</a>
                {/if}
            {/if}
        </div>
    {/if}
                        ...
                        <div class="callbacks_container">
                            <div class="ioutline" style="max-width:400px;margin:0 auto;">
                                <ul id="slider1" class="rslides callbacks" style="max-height:400px;">
                                    {if isset($_conf.jrProJamLight_profile_ids) && strlen($_conf.jrProJamLight_profile_ids) > 0}
                                        {jrCore_list module="jrProfile" order_by="_created desc" limit="10" search1="_profile_id in `$_conf.jrProJamLight_profile_ids`" search2="profile_active = 1" template="index_featured_slider.tpl"}
                                    {else}
                                        {if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
                                            {if isset($_conf.jrProJamLight_artist_quota) && $_conf.jrProJamLight_artist_quota > 0}
                                                {jrCore_list module="jrProfile" order_by="_created desc" limit="10" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" template="index_featured_slider.tpl" require_image="profile_image"}
                                            {else}
                                        ...
                                        {else}
                                            {if isset($_conf.jrProJamLight_artist_quota) && $_conf.jrProJamLight_artist_quota > 0}
                                                {jrCore_list module="jrProfile" order_by="_created desc" limit="10" search1="profile_active = 1" search2="profile_quota_id in `$_conf.jrProJamLight_artist_quota`" template="index_featured_slider.tpl"}
                                            {else}
