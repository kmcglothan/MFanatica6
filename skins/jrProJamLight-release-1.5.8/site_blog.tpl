<div style="height:588px;overflow:auto;">
                ...
                <table style="width:100%;">
                    <tr>
                        ...
                        <td class="body_5 page" style="width:25%;text-align:center;">
                            ...
                            <a onclick="jrLoad('#site_news_div','{$jamroom_url}/site_blog/p={$info.prev_page}');$('html, body').animate({ scrollTop: $('#mnews').offset().top -100 }, 'slow');"><span class="button-arrow-previous">&nbsp;</span></a>
                            {else}
                        ...
                        <td class="body_5" style="width:50%;text-align:center;">
                            {if $info.total_pages <= 3 || $info.total_pages > 20}
                            {$info.page} &nbsp;/ {$info.total_pages}
                            {else}
                            <form name="form" method="post" action="_self">
                                <select name="pagenum" class="form_select" style="width:60px;" onchange="var sel=this.form.pagenum.options[this.form.pagenum.selectedIndex].value;jrLoad('#site_news_div','{$jamroom_url}/site_blog/p=' +sel);$('html, body').animate({ scrollTop: $('#mnews').offset().top -100 }, 'slow');">
                        ...
                        <td class="body_5 page" style="width:25%;text-align:center;">
                            ...
                            <a onclick="jrLoad('#site_news_div','{$jamroom_url}/site_blog/p={$info.next_page}');$('html, body').animate({ scrollTop: $('#mnews').offset().top -100 }, 'slow');"><span class="button-arrow-next">&nbsp;</span></a>
                            {else}
    ...
    {* EVENT LIST FUNCTION *}
    {if isset($_conf.jrProJamLight_blog_profile) && strlen($_conf.jrProJamLight_blog_profile) > 0}
        {jrCore_list module="jrBlog" order_by="_created desc" search1="_profile_id in `$_conf.jrProJamLight_blog_profile`" search2="blog_category = blog" template=$site_blog_template pagebreak=$_conf.jrProJamLight_index_blog_limit page=$_post.p}
    {else}
        {jrCore_list module="jrBlog" order_by="_created desc" search1="_profile_id = 1" search2="blog_category = blog" template=$site_blog_template pagebreak=$_conf.jrProJamLight_index_blog_limit page=$_post.p}
    {/if}
</div>
