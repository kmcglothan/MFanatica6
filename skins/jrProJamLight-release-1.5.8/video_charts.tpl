                        ...
                        <div class="col12 last">
                            ...
                            <div class="body_3">
                                ...
                                {if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
                                    {if isset($_post.search_area) && $_post.search_area == 'video_category'}
                                        {jrCore_list module="jrVideo" chart_field="video_file_stream_count" chart_days="7" search="video_category like `$_post.search_string`" template="video_chart_row.tpl" require_image="video_image" pagebreak=$_conf.jrProJamLight_default_pagebreak page=$_post.p}
                                    {else}
                                        {jrCore_list module="jrVideo" chart_field="video_file_stream_count" chart_days="7" template="video_chart_row.tpl" require_image="video_image" pagebreak=$_conf.jrProJamLight_default_pagebreak page=$_post.p}
                                    {/if}
                                {else}
                                    {if isset($_post.search_area) && $_post.search_area == 'video_category'}
                                        {jrCore_list module="jrVideo" chart_field="video_file_stream_count" chart_days="7" search="video_category like `$_post.search_string`" template="video_chart_row.tpl" pagebreak=$_conf.jrProJamLight_default_pagebreak page=$_post.p}
                                    {else}
                                        {jrCore_list module="jrVideo" chart_field="video_file_stream_count" chart_days="7" template="video_chart_row.tpl" pagebreak=$_conf.jrProJamLight_default_pagebreak page=$_post.p}
                                    {/if}
                                {/if}
                            </div>
                        </div>
