...
<table class="menu_tab">
    <tr>
        {if isset($_conf.jrProJamLight_v_category) && $_conf.jrProJamLight_v_category == 'on'}
            {if isset($_post.search_area) && $_post.search_area == 'video_category'}
                ...
                <td class="spacer">&nbsp;</td>
                <td>
                    <div id="default" class="p_choice" onclick="jrLoad('#stats','{$jamroom_url}/video_chart_categories{if isset($period) && strlen($period) >0}/{$period}{/if}');jrSetActive('#default');">{jrCore_lang skin=$_conf.jrCore_active_skin id="167" default="Genres"}</div>
                </td>
            {else}
                <td>
                ...
                <td class="spacer">&nbsp;</td>
                <td>
                    <div id="video_genre" class="p_choice" onclick="jrLoad('#stats','{$jamroom_url}/video_chart_categories{if isset($period) && strlen($period) >0}/{$period}{/if}');jrSetActive('#video_genre');">{jrCore_lang skin=$_conf.jrCore_active_skin id="167" default="Genres"}</div>
                </td>
            {/if}
        {else}
            <td>
...
<div class="body_3 mb20">
    ...
    {if isset($_conf.jrProJamLight_todays_featured) && $_conf.jrProJamLight_todays_featured > 0}
        {jrCore_list module="jrProfile" limit="1" search2="_profile_id = `$_conf.jrProJamLight_todays_featured`" template=$featured_today}
    {else}
        {if isset($_conf.jrProJamLight_require_images) && $_conf.jrProJamLight_require_images == 'on'}
            {if isset($_conf.jrProJamLight_artist_quota) && strlen($_conf.jrProJamLight_artist_quota) > 0}
                {jrCore_list module="jrProfile" order_by="_created random" limit="1" search1="profile_active = 1" quota_id=$_conf.jrProJamLight_artist_quota template=$featured_today require_image="profile_image"}
            {else}
        ...
        {else}
            {if isset($_conf.jrProJamLight_artist_quota) && strlen($_conf.jrProJamLight_artist_quota) > 0}
                {jrCore_list module="jrProfile" order_by="_created random" limit="1" search1="profile_active = 1" quota_id=$_conf.jrProJamLight_artist_quota template=$featured_today}
            {else}
...
{* HOUSE STATION *}
{if isset($selected) && ($selected == 'music' || $selected == 'galleries')}
    {if isset($_conf.jrProJamLight_show_radio) && $_conf.jrProJamLight_show_radio == 'on'}
        {if isset($_conf.jrProJamLight_radio_title) && strlen($_conf.jrProJamLight_radio_title) > 0}
            {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.jrProJamLight_radio_title`" limit="1" template="index_radio.tpl"}
        {else}
            {if jrUser_is_logged_in()}
                {if jrUser_is_master()}
                    ...
                    <div class="body_2 normal p20 mb20">
                        Admin Note:&nbsp;<a href="{$jamroom_url}/core/skin_admin/global/skin=jrProJamLight">Settings</a> <b>"Radio Title"</b> is not set!
                    </div>
                {/if}
            {/if}
        {/if}
...
{if isset($selected) && ($selected == 'videos' || $selected == 'galleries')}
{* HOUSE CHANNEL *}
    {if isset($_conf.jrProJamLight_show_tv) && $_conf.jrProJamLight_show_tv == 'on'}
        {if isset($_conf.jrProJamLight_tv_title) && strlen($_conf.jrProJamLight_tv_title) > 0}
            {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.jrProJamLight_tv_title`" tpl_dir="jrProJamLight" template="index_channel.tpl"}
        {else}
            {if jrUser_is_logged_in()}
                {if jrUser_is_master()}
...
<a id="t10vcharts" name="t10vcharts"></a>
{* TOP 10 VIDEO CHARTS *}
...
{if isset($TOP_TEN_VIDEO_CHARTS) && strlen($TOP_TEN_VIDEO_CHARTS) > 0}
    <h3>Top 10 Video Charts</h3><br>
    <br>
    <div id="side_video_charts" style="height:560px;">
...
{* OUR SPONSORS *}
{if $_conf.jrProJamLight.ads_off != 'on'}
    <div class="body_2 center mt20 mb20">
        {if isset($_conf.jrProJamLight_google_ads) && $_conf.jrProJamLight_google_ads == 'on'}
            <script type="text/javascript"><!--
                google_ad_client = "{$_conf.jrProJamLight_google_id}";
                google_ad_width = 180;
        ...
        {elseif isset($_conf.jrProJamLight_side_ad) && strlen($_conf.jrProJamLight_side_ad) > 0}
            {$_conf.jrProJamLight_side_ad}
        {else}
