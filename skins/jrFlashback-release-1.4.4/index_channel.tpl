...
{if isset($_items)}
    ...
    <div class="title">
        <h2>{jrCore_lang skin=$_conf.jrCore_active_skin id="42" default="house"} {jrCore_lang skin=$_conf.jrCore_active_skin id="44" default="channel"}</h2>
    </div>
    <div class="body_1 mb20">
        <div style="display:table">
            ...
            <div style="display:table-cell;text-align:center;vertical-align:middle;">
                <a onclick="popwin('{$jamroom_url}/channel_player/playlist_id={$item._item_id}/title={$item.playlist_title}','channel_player',805,625,true);">{jrCore_image image="button_player_play.png" width="32" height="32" alt="{$item.playlist_title}" title="{$item.playlist_title}" onmouseover="$(this).attr('src','`$jamroom_url`/skins/`$_conf.jrCore_active_skin`/img/button_player_play_hover.png');" onmouseout="$(this).attr('src','`$jamroom_url`/skins/`$_conf.jrCore_active_skin`/img/button_player_play.png');"}</a>
            </div>
        </div>
    </div>
    {/foreach}
